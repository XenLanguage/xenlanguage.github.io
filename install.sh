...
linux() {
    ...
    curl "https://github.com/jakerieger/Xen/releases/download/v0.5.2/Xen-0.5.2-linux-x64.tar.gz" > "$temp/Xen-0.5.2-linux-x64.tar.gz"
    rm -rf "$HOME/.local/xen"
    mkdir -p "$HOME/.local/xen"
    tar -xzf "$temp/Xen-0.5.2-linux-x64.tar.gz" -C "$HOME/.local/xen/"
    rm -f "$temp/Xen-0.5.2-linux-x64.tar.gz"
}
